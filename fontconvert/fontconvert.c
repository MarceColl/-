	...
	*num_items = 0;
	...
	for (int i = 0; i < arg_len; i++) {
		if (*(arg + i) == ';') {
			*(arg + i) = 0;	
			if (first == -1) {
				values[*num_items] = atoi(cptr);

				if (values[*num_items] > *max) {
					*max = values[*num_items];
				} 

				if (values[*num_items] < *min) {
					*min = values[*num_items];
				}
...
int main(int argc, char *argv[]) {
  int i, j, err, size, first = INT_MAX, last = INT_MIN, bitmapOffset = 0, x, y, byte;
  char *fontName, c, *ptr;
  FT_Library library;
  FT_Face face;
  FT_Glyph glyph;
  ...
  if (argc == 4) {
    values = expand_range(argv[3], &num_items, &last, &first);
  } 
  ...
  if ((!(fontName = malloc(strlen(ptr) + 20))) ||
      (!(table = (GFXglyph *)malloc(num_items * sizeof(GFXglyph))))) {
